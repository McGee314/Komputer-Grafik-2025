	...
	public List<Vector2> Lingkaran(int xCenter, int yCenter, int radius)
{
	...
	while (x <= y) // <= instead of <
	{
		...
		// Ensure all octants are plotted
		CirclePlotPoints(xCenter, yCenter, x, y, points);
	}
	...
	private void CirclePlotPoints(int xc, int yc, int x, int y, List<Vector2> points)
	{
		points.Add(new Vector2(xc + x, yc + y));
		points.Add(new Vector2(xc - x, yc + y));
		points.Add(new Vector2(xc + x, yc - y));
		points.Add(new Vector2(xc - x, yc - y));
		points.Add(new Vector2(xc + y, yc + x));
		points.Add(new Vector2(xc - y, yc + x));
		points.Add(new Vector2(xc + y, yc - x));
		points.Add(new Vector2(xc - y, yc - x));
	}
